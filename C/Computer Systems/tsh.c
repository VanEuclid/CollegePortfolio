...
//globals I put in
static int fg_pid; //handles foreground process
static sigset_t blocked; //handles blocked signals
static sigset_t empty; //handles empty signal for suspend
static int sigtstopFlag; //handles printing for sigtstp
/*
...
int main(int argc, char **argv) 
{
  ...
  sigemptyset(&empty); //setup sigsuspender
...
void eval(char *cmdline) 
{
  ...
  if(builtin_cmd(argv1) == 0 && cmd2 == NULL){ //Cmd is not builtin

    sigemptyset(&blocked);
    sigaddset(&blocked, SIGCHLD);
    sigprocmask(SIG_BLOCK, &blocked, NULL); //SETUPS SIGNAL BLOCKER

    pid_t pid = fork();
    if(pid == 0){ //In child process
      setpgid(0, 0); //Sets new group pgid for child
	...
	struct job_t *tempjob = getjobpid(jobs, pid);
...
int parseline(const char *cmdline, char **argv, int cmdnum) 
{
  ...
  //printf("1 for background, 0 for foreground = %d\n", bg);    
  return bg;
}
...
void do_bg(int jid) 
{
  struct job_t *tempjob = getjobjid(jobs, jid);
...
void sigchld_handler(int sig) 
{
  ...
  while((finished_pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0){
    ...
    if(WTERMSIG(status) == 2){
      struct job_t *tempjob = getjobpid(jobs, finished_pid);
      printf("Job [%d] (%d) terminated by signal 2\n", tempjob->jid, tempjob->pid);
	...
	int targetJob = pid2jid(finished_pid);
	jobs[targetJob-1].state = ST;
      }
    }
    else {
      deletejob(jobs, finished_pid);
...
void sigtstp_handler(int sig) 
{
  if(fg_pid != 0) {
    kill(-fg_pid, SIGTSTP);
    struct job_t *tempjob = getjobpid(jobs, fg_pid);
    printf("Job [%d] (%d) stopped by signal 20\n", tempjob->jid, tempjob->pid);

    sigtstopFlag = 1;
    int targetJob = pid2jid(fg_pid);
    jobs[targetJob-1].state = ST;
    fg_pid = 0;
  }
