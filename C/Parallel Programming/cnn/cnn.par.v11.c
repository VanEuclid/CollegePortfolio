...
#pragma omp parallel for private(ij, ii, n, k, q, p, c, r, s, sum) schedule(dynamic)
    for (n=0; n<N; n++) { // minibatch size

#pragma omp parallel for private(ij, ii, k, q, p, c, r, s, sum) collapse(2)
      for (k=0; k<K; k ++) { // output feature map

	//#pragma omp parallel for private(ij, ii, q, p, c, r, s, sum)
        for (c=0; c<C; c ++) { // input feature map

#pragma omp parallel for private(ij, ii, q, p, r, s, sum) schedule(static, 4) num_threads(4)
          for (p=0; p<P; p ++) { // output height
            ij = p * u; // input height

//#pragma omp parallel for private(ii, q, r, s)
#pragma omp parallel for reduction(+:sum) private(ii, q, r, s)
            for (q =0; q<Q; q ++) { // output width
              ii = q * v; // input width
	      sum = output_par[n][k][p][q];

	      for (r=0; r<R; r ++) { // filter height

//#pragma omp parallel for reduction(+:sum) private(s)
	        for (s =0; s< S; s ++) {// filter width
	        sum += input [n][c][ij+r][ii+s] * weight[k][c][r][s];
		} }
