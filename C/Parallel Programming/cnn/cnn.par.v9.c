...
#pragma omp parallel for private(ij, ii, n, k, q, p, c, r, s, sum)
    for (n=0; n<N; n++) { // minibatch size
...
#pragma omp parallel for private(ij, ii, q, p, r, s, sum) schedule(dynamic)
          for (p=0; p<P; p ++) { // output height
            ij = p * u; // input height

//#pragma omp parallel for private(ii, q, r, s)
#pragma omp parallel for reduction(+:sum) private(ii, q, r, s) schedule(dynamic)
            for (q =0; q<Q; q ++) { // output width
              ii = q * v; // input width
