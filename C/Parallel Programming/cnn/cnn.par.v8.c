...
#pragma omp parallel for private(ij, ii, n, k, q, p, c, r, s, sum) collapse(3)
    for (n=0; n<N; n++) { // minibatch size

      //#pragma omp parallel for private(ij, ii, k, q, p, c, r, s, sum)
      for (k=0; k<K; k ++) { // output feature map

	//#pragma omp parallel for private(ij, ii, q, p, c, r, s, sum)
        for (c=0; c<C; c ++) { // input feature map

#pragma omp parallel for private(ij, ii, q, p, r, s, sum)
          for (p=0; p<P; p ++) { // output height
            ij = p * u; // input height
