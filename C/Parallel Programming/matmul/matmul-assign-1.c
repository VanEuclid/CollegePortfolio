...
void Calc1(float myc[N/P][N], float mya[N/P][N], float myb[N/P][N], int rank) {
  ...
  for (i = 0; i< N/P; i++) {
	...
	myc[i][j] += mya[i][rank*(N/P)+ k] * myb[k][j];
	//printf("in Rank %d myc[%d][%d] += mya[%d][%d] * myb[%d][%d]\n", rank, i, j, i, rank*(N/P)+k, k, j);
      }
    }
  }
  //printf("finished Calc1 from rank = %d\n", rank);
}
...
void Calc2(float myc[N/P][N], float mya[N/P][N], float tmpb[N/P][N], int rank) {
  ...
  for (i = 0; i< N/P; i++) {
    for (j = 0; j<N; j++) {
      for (k=0; k<N/P; k++) {
	myc[i][j] += mya[i][rank*(N/P) + k] * tmpb[k][j];
  ...
  //printf("finished Calc2 from rank = %d\n", rank);
}
