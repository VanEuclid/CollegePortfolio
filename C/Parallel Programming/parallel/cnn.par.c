...
#pragma omp parallel for private(nn, kk, p, ij, n, k, q, ii, sum, c, r, s) collapse(2) schedule(dynamic)
  for(nn = 0; nn < N; nn += tS) {
    for(kk = 0; kk <K; kk += tS) {
      for (p=0; p<P; p ++) {
	ij = p * u;
#pragma omp parallel for private(n, k, q, ii, sum, c, r, s) collapse(2) schedule(dynamic)
	for(n = nn; n < min(N, nn+tS); n++) {
	  for(k = kk; k < min(K, kk+tS); k++) {
	    for (q =0; q<Q; q ++) {
	      ii = q * v;
