...
void computeJointTransformations(
	const std::vector<Matrix4f>& p_local, //t
	const std::vector<Matrix4f>& p_offset, //r
	const std::vector<int>& p_jointParent,
	const unsigned int p_numJoints,
	std::vector<Matrix4f>& p_global) //f
{
	// TASK 1 comes here
	Matrix4f r;
	r.setIdentity();
	bool tCheckIden = true;
	if (p_local[p_numJoints - 1] == r) {
		for (unsigned int check = 0; check < p_numJoints; check++) {
			if (p_local[check] != r) {
				tCheckIden = false;
				break;
			}
		...
		if (tCheckIden) {
			p_global[0] = p_offset[0];
			for (unsigned int frameId = 1; frameId < p_numJoints; frameId++) {
				p_global[frameId] = p_global[p_jointParent[frameId]] * p_offset[frameId];
			}	
		}
		else {
			p_global[0] = p_offset[0] * p_local[0];
			for (unsigned int frameId = 1; frameId < p_numJoints; frameId++) {
				p_global[frameId] = p_global[p_jointParent[frameId]] * p_offset[frameId] * p_local[frameId];
			}
		}
	}
	else {
		...
		std::vector<Vector3f>& p_deformedVertices)
{
	...
	for (unsigned int v = 0; v < p_vertices.size(); v++) {
		...
		for (unsigned int jointId = 0; jointId < p_numJoints; jointId++) {
			p_deformedVertices[v] += fromHomog(p_weights[jointId][v] * ((p_jointTrans[jointId] * g_jointTransRestInv[jointId]) * toHomog(p_vertices[v])));
		}
 	}
}
