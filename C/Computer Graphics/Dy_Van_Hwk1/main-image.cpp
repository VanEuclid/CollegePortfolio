...
void compress(float* C, int blockSize, int m)
{
	...
	int compressI = blockSize + blockSize - 2 - m;
	for (int i = 0; i < blockSize; i++) {
		for (int j = 0; j < blockSize; j++) {
			printf("C[%d %d] compression result =  %d\n", i, j, (i + j) > compressI);
			if ((i + j) > compressI) {
				C[i * 8 + j] = 0;
			}
		}
...
int main()
{
	...
	int m = 14; // TODO: change the parameter m from 1 to 16 to see different image quality (14 is max compression anything past that is black)
