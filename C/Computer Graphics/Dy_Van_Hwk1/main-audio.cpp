...
void compress(float* C, int size, int m)
{
	...
	int startCompress = size - m;
	for (int compressI = 0; compressI < size; compressI++) {
		if (compressI >= startCompress) {
			*(C + compressI) = 0;
		}
	}

	//std::cout << "FROM C COMPRESS" << std::endl;
	//for (int index = 0; index < size; index++) {
	//	printf("C[%d] =  %f\n", index, *(C + index));
	//}
	//std::cout << "END C" << std::endl;
...
void inverseDCT(const float* C, float* B, int size)
{
	...
	float cu, sum;
	for (int i = 0; i < size; i++) { //outter loop to collect F(u)
		sum = 0;
		for (int u = 0; u < size; u++) {
			if (u == 0) { //logic to determine sk
				cu = sqrtf(8) / 8;
			}
			else {
				cu = sqrtf(2) / sqrtf(8);
			}
			sum += cu * std::cos(((2 * i + 1) * u * M_PI) / (2 * size)) * C[u]; //dot product
		}
		B[i] = sum;
	}

	//std::cout << "FROM B INVERSE" << std::endl;
	//for (int index = 0; index < size; index++) {
	//	printf("B[%d] =  %f\n", index, *(B + index));
	//}
...
void processWAVSignal()
{
	...
	for (int i = 0; i < g_wav_size; i += 8) //PROCESSES BLOCK PORTIONS HERE, moves i 8 positions for 8 floats
	{
		A = g_wav_data + i;
